...
import { Target, Quote, Activity, Zap, Cpu, Download, Database, Ruler } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const chartValues = [
    ...
    { name: Subject.MATHEMATICS, value: progress.mathScore, color: '#818cf8' },
  ];

  const hasData = chartValues.some(d => d.value > 0);
  const chartData = hasData 
    ? chartValues 
    : [{ name: 'Awaiting IIT Solves', value: 1, color: '#1e293b' }];
  ...
  return (
    <div id="stats-section" className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-16 px-0 w-full overflow-hidden">
      ...
      <div className="lg:col-span-3 glass-card heartbeat-glow p-6 md:p-8 rounded-[2rem] flex flex-col md:flex-row items-center gap-6 md:gap-8 relative overflow-hidden w-full">
        ...
        <div className="relative z-10 flex-1 text-center md:text-left">
            <div className="flex items-center gap-2 mb-2 justify-center md:justify-start">
               ...
               <h3 className="text-blue-400 font-bold text-[10px] uppercase tracking-[0.3em]">JEE Protocol: ACTIVE</h3>
            </div>
            <p className="text-slate-100 text-lg md:text-2xl font-semibold leading-snug italic">
              "{progress.dailyQuote || "The logic of today builds the foundation for your IIT seat."}"
            </p>
        </div>
        <div className="absolute -right-10 top-1/2 -translate-y-1/2 opacity-5 pointer-events-none">
          <Ruler size={200} className="text-blue-500" />
        </div>
      </div>
      ...
      <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <div className="glass-card p-6 md:p-10 rounded-[2.5rem] border-blue-500/20 bg-gradient-to-br from-slate-900/80 to-slate-950 flex flex-col justify-center relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-400 text-sm font-medium">High-yield JEE problems mastered</p>
        ...
        <div className="glass-card p-6 md:p-8 rounded-[2.5rem] flex flex-col items-center justify-center min-h-[300px]">
          <h3 className="text-slate-400 font-bold mb-4 text-[10px] uppercase tracking-widest self-start">Neural Sync Vitals</h3>
          <div className="w-full h-[180px] md:h-[220px] relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-2">
              ...
              <span className="text-3xl font-bold text-white leading-none">
                {progress.physicsScore + progress.chemistryScore + progress.mathScore}
              </span>
            </div>
          </div>
      ...
      <div className="h-full lg:col-span-1">
       ...
       <div className="bg-gradient-to-r from-blue-900/30 via-slate-900/30 to-indigo-900/20 glass-card p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] lg:col-span-3 flex items-center justify-between border-blue-500/10 gap-4">
         <div className="flex-1">
            <h3 className="font-extrabold text-xl md:text-2xl mb-2 text-white italic flex items-center gap-3">
              Engineer in Training: Cheenu <Cpu size={24} className="text-blue-400" />
            </h3>
            <p className="text-slate-400 text-[11px] md:text-sm max-w-2xl font-medium leading-relaxed">
                "Logic is your greatest weapon, Cheenu. Your progress is kept safe in your browser storage. Every calculus milestone you complete tonight brings you closer to your dream IIT branch. I'm ALOO, and I've locked your data in."
            </p>
         </div>
