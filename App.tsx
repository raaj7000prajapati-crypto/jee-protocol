...
const App: React.FC = () => {
  const [progress, setProgress] = useState<UserProgress>(() => {
    const defaultState: UserProgress = {
      ...
      dailyQuote: "The logic you build today creates the world of tomorrow.",
      lastQuoteDate: '',
      seenQuestions: [],
      tasks: [],
  ...
  // Data is saved automatically in the browser's long-term memory.
  useEffect(() => {
      ...
      if (now.getHours() === 20 && now.getMinutes() === 0) {
        ...
        if (incompleteTasks > 0) {
            new Notification("JEE Prep Reminder", {
                body: `Hey Cheenu, you have ${incompleteTasks} milestones left for today! Let's finish them.`,
                icon: '/favicon.ico'
            });
        }
      }
  ...
  const handleCorrectAnswer = (subject: Subject) => {
    setProgress(prev => {
      ...
      if (subject === Subject.MATHEMATICS) newProgress.mathematicsScore += 1;
      ...
      <footer className="mt-20 text-center border-t border-slate-900 pt-12 pb-12">
        <div className="flex flex-col items-center gap-4">
            <div className="flex gap-2 items-center">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500">Mission: Cheenu IITian (JEE 2026)</span>
                <div className="w-2 h-2 rounded-full bg-blue-600 animate-pulse"></div>
            </div>
            <p className="text-slate-600 text-[10px] font-bold uppercase tracking-widest">© 2025 Neural Core Prep. Engineered for Cheenu.</p>
        </div>
      </footer>
    </div>
  );
